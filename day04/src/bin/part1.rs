...
fn play_to_first_win(mut boards: Vec<Board>, draws: &[i32]) -> Option<i32> {
    for &n in draws {
        for b in boards.iter_mut() {
            if !b.has_won && b.mark_and_check(n) {
                return Some(b.sum_unmarked() * n);
            }
        }
    }
    None
...
fn main() -> Result<()> {
    ...
    if let Some(score) = play_to_first_win(boards, &draws) {
        println!("Part 1 (first winner) score: {score}");
    } else {
